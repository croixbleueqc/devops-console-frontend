...
export class BuildOpenapiClient implements BuildTask {
  constructor(readonly aspectId: string) {}

  readonly name = 'BuildOpenapiClient';
  ...
  async execute(context: BuildContext): Promise<BuiltTaskResult> {
    ...
    capsules.forEach((capsule) => {
      ...
      try {
        ...
        fs.writeFileSync(path.join(capsuleDir, 'output.my-artifact.txt'), artifactContent);
      } catch (err: any) {
        errors.push(err);
      }
